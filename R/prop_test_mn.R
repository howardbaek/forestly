...
#'   4. For non-inferiority or one-sided equivalence hypothesis, the p-value is
#'      computed based on the test-statistic (Zdiff) using the standard normal
#'      distribution.
#'   5. For superiority or two-sided test, the p-value is computed based on the
#'      test statistic (Zdiff^2) using the Chi-square distribuiton with df = 1.
#'
...
#' @param sides  specify the hypothesis directions: two.sided, righ(less than),
#'               left(greater than)
...
prop_test_mn <- function(x0, n0, x1, n1,
  ...
  if (sides == "right" | sides == "left") {
    pvalue <- ifelse(delta <= 0, 1 - stats::pnorm(z_stats),
                     stats::pnorm(z_stats))
