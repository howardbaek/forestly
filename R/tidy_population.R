...
#' The function filter the `adsl` dataset, and define proper treatment order
#' in the dataset.
#'
...
#' @param population_where A character string to define the criteria to select
#'                         analysis population.
...
#'                        It also provide the label names of the treatments
#'                        after turning them into factors.
#' @param stratum_var A character string to define the variable of baseline
#'                    stratum in 'population_from'.
#'                    Only one 'stratum_var' is allowed.
#' @param baseline_var Tells the function which column of the table to be
...
tidy_population <- function(population_from,
  ...
  pop <- pop[pop$treatment %in% treatment_order, ]
  ...
  pop[["treatment"]] <- factor(pop[["treatment"]], levels = treatment_order,
                               labels = label_name)
