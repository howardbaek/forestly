...
with Miettinen and Nurminen method (M&N) for risk difference analysis R. 
The following statistics can be calculated within the function: 
...
Also this approach is applicable to the case when the data are collected using a stratified clinical design and
...
The confidence interval is based on by the M&N method and given by the roots for $PD=P_1-P_0$ of the equation:  
...
for $P_1$ and $P_0$ respectively, $S_0$ is pre-specified proprotion difference under the null; 
...
Within each stratum there are two independent binomial samples with binary response variables to be analyzed/summarized. 
The parameter of interest is the difference between the population proportions of the test and the control groups. 
...
>>>>>>> c44f43a896ab750dac7592ed180df18821f54e37
```{r message=FALSE}
library(dplyr)
library(tidyr)
library(survival)
